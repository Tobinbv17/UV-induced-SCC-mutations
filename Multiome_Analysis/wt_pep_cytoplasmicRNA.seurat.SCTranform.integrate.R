...
library(circlize)
#1.
QV1G12V_wt = Read10X('/home/yao.zhan/analysis-CRv4/nextseq0057-RNA-QV1G12V-WT-cyt/outs/filtered_feature_bc_matrix/')
QV1G12V_pep = Read10X('/home/yao.zhan/analysis-CRv4/nextseq0058-RNA-QV1G12V-pep-cyt/outs/filtered_feature_bc_matrix/')
...
saveRDS(immune.combined.sct, 'MultiomeSeq/cytoplasmic_RNA/combined.sct.rds')
###############################################################################
...
write.csv(group_marker, 'MultiomeSeq/cytoplasmic_RNA/DEG-wt_vs_pep_cytoplasmic.csv')

ggplot(group_marker, aes(avg_log2FC, -log10(p_val_adj))) + geom_point(size=.2) + 
  ggtitle('Cytoplasmic DEG in QV1G12V-WT Vs pep ')
...
#### 20210623 update 2x up and 2x down gene
...
pdf('MultiomeSeq/cytoplasmic_RNA/2X_up_DEG_pepVSwt.pdf')
hmap <- Heatmap((df1),name="Expression",col <- colorRamp2(c(-2,0,2),c("blue","white", "red")),
                ...
                top_annotation = HeatmapAnnotation(sample = rep(c('QV1G12V-Wt','QV1G12V-Pep'),times=c(8924,6802)),
